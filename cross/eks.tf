...
module "eks" {
  ...
  cluster_endpoint_public_access_cidrs = ["67.202.24.21/32"] #10.100.120.83/32"]
  cluster_addons = {
    coredns = {
      most_recent = true
    }
  ...
  eks_managed_node_group_defaults = {
    ami_type                              = "AL2_x86_64"
    instance_types                        = ["t3.large", "m5.large", "m6i.large"] ##"m6i.large", "m5zn.large", "m5n.large", 
  ...
  self_managed_node_groups = {
    ...
    # wl-eks-cross node group
    wl-eks-cross = {
      name = "wl-eks-cross-self-mng"

      platform      = "wl-eks-cross"
      ami_id        = data.aws_ami.eks_default.id #data.aws_ami.eks_default_wl-eks-cross.id
      instance_type = "t3.large"
      desired_size  = 1
      key_name      = data.aws_key_pair.cross.key_name #module.key_pair.key_pair_name
...
#data "aws_ami" "eks_default_wl-eks-cross" {
#  most_recent = true
#  owners      = ["amazon"]
#
#  filter {
#    name   = "name"
#    values = ["wl-eks-cross-aws-k8s-${local.cluster_version}-x86_64-*"]
#  }
#}
